...
class ArrowKeyTeleop(Node):
    ...
    def setup_gui(self):
        ...
        speed_font = font.Font(family="Courier", size=14)
        
        # Title
        tk.Label(self.root, text="Robot Teleop Control", font=title_font, bg="#f0f0f0").pack(pady=(15, 10))
        
        # Current speed display
        self.speed_frame = tk.Frame(self.root, bg="#f0f0f0")
        self.speed_frame.pack(pady=15)
        
        tk.Label(self.speed_frame, text="Linear X:", font=speed_font, bg="#f0f0f0").grid(row=0, column=0, sticky="e", padx=(0, 10))
        self.linear_speed_label = tk.Label(self.speed_frame, text="0.0 m/s", font=speed_font, bg="#e6e6e6", width=12, relief="sunken", padx=5, pady=3)
        self.linear_speed_label.grid(row=0, column=1, sticky="w", pady=5)
        
        tk.Label(self.speed_frame, text="Angular Z:", font=speed_font, bg="#f0f0f0").grid(row=1, column=0, sticky="e", padx=(0, 10))
        self.angular_speed_label = tk.Label(self.speed_frame, text="0.0 rad/s", font=speed_font, bg="#e6e6e6", width=12, relief="sunken", padx=5, pady=3)
        self.angular_speed_label.grid(row=1, column=1, sticky="w", pady=5)

        tk.Label(self.speed_frame, text="Policy:", font=speed_font, bg="#f0f0f0").grid(row=2, column=0, sticky="e", padx=(0, 10))
        self.policy_label = tk.Label(self.speed_frame, text="OFF", font=speed_font, bg="#e6e6e6", width=12, relief="sunken", padx=5, pady=3)
        self.policy_label.grid(row=2, column=1, sticky="w", pady=5)
        
        # Status display
        self.status_label = tk.Label(self.root, text="Status: Click window to activate", font=info_font, bg="#fff3cd", fg="#856404", padx=10, pady=8)
        self.status_label.pack(fill="x", padx=20, pady=10)
        
        # Controls instructions
        controls_frame = tk.Frame(self.root, bg="#f0f0f0", relief="ridge", bd=1)
        controls_frame.pack(fill="both", expand=True, padx=20, pady=(5, 20))
        ...
        tk.Label(controls_frame, text="Controls:", font=info_font, bg="#f0f0f0").grid(row=0, column=0, columnspan=2, sticky="w", pady=(8, 10), padx=10)
        ...
        for i, (key, desc) in enumerate(instructions):
            ...
            key_label = tk.Label(controls_frame, text=key, font=speed_font, bg=bg_color, width=8, padx=5, pady=5)
            key_label.grid(row=i+1, column=0, sticky="w", padx=10, pady=6)
            
            desc_label = tk.Label(controls_frame, text=desc, font=info_font, bg="#f0f0f0", anchor="w", padx=5)
            desc_label.grid(row=i+1, column=1, sticky="w", padx=10, pady=6)
        ...
        try:
            ...
            while True:
                ...
                if ch == '\x1b':  # Arrow keys start with ESC
                    ...
                    break
                elif ch in ['0', '1', '2', 'i', 'k', 'r', 'c', 'p', 't', 'f', 'g', 'b', 's', 'e', 'o', 'n', 'm',
                            'I', 'K', 'R', 'C', 'P', 'T', 'F', 'G', 'B', 'S', 'E', 'O', 'N', 'M']:
    ...
    def update_controls_instructions(self):
        ...
        if hasattr(self, 'root') and self.root.winfo_exists():
            ...
            instructions = [
                ...
                ("i/k", "Increase/Decrease torso height"),
                ("r/rr", "Reset arm / controller"),
                ("c", "Collect Scan"),
                ("p", "On/OFF Policy"),
                ("t/f", "Save Data"),
                ("g", "Slightly Release Gripper"),
                ("b", "Clean Stitched Buffer"),
                ("s", "Predict SIR"),
                ("e", "Execute (Torso Only)"),
                ("o", "Generate Random Pose"),
                ("n", "Change Row"),
                ("m", "Change Column"),
            ...
            for i, (key, desc) in enumerate(instructions):
                ...
                if key in ["r/rr", "c", "p", "t", "t/f", "g", "b", "s", "e", "o", "n", "m"]:
            ...
            feedback_frame.grid(row=len(instructions)+1, column=0, columnspan=2, sticky="ew", padx=10, pady=(20, 6))
            
            tk.Label(feedback_frame, text="Command Status:", font=info_font, bg="#f0f0f0").pack(anchor="w")
            self.feedback_label = tk.Label(feedback_frame, text="Ready", font=speed_font, bg="#e6e6e6", padx=5, pady=3, width=40, anchor="w")
